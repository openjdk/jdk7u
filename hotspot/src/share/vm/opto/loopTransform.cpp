...
bool PhaseIdealLoop::intrinsify_fill(IdealLoopTree* lpt) {
  ...
  if (SharedRuntime::c_calling_convention_requires_ints_as_longs() &&
      // see StubRoutines::select_fill_function for types. FLOAT has been converted to INT
      (t == T_FLOAT || t == T_INT || t == T_SHORT || t == T_CHAR || t == T_BOOLEAN || t == T_BYTE)) {
  ...
  // PPC64 needs top here as later we will set MachConstantBase.
#ifdef PPC64
