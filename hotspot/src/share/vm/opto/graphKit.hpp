...
class GraphKit : public Phase {
 ...
 public:
  ...
  Node* make_load(Node* ctl, Node* adr, const Type* t, BasicType bt,
                  bool require_atomic_access = false, LoadNode::Sem sem = LoadNode::unordered) {
    // This version computes alias_index from bottom_type
    return make_load(ctl, adr, t, bt, adr->bottom_type()->is_ptr(),
                     require_atomic_access, sem);
  }
  ...
  Node* store_to_memory(Node* ctl, Node* adr, Node* val, BasicType bt,
                        ...
                        StoreNode::Sem sem = StoreNode::unordered) {
    // This version computes alias_index from an address type
  ...
  Node* store_to_memory(Node* ctl, Node* adr, Node* val, BasicType bt,
                        int adr_idx,
                        bool require_atomic_access = false,
                        StoreNode::Sem = StoreNode::unordered);
  ...
  Node* store_oop_to_object(Node* ctl,
                            ...
                            BasicType bt,
                            StoreNode::Sem sem = StoreNode::unordered) {
    return store_oop(ctl, obj, adr, adr_type, val, val_type, bt, false, sem);
  }
