...
#define BI_PROFILE_PRINT_METHOD_DATA()                                         \
  {                                                                            \
    ttyLocker ttyl;                                                            \
    methodDataOop mdo = istate->method()->method_data();                       \
    tty->cr();                                                                 \
    ...
    if (mdo == NULL) {                                                         \
      ...
      if (INVOCATION_COUNT->reached_ProfileLimit(BACKEDGE_COUNT)) {            \
        ...
        if (mdo != NULL) {                                                     \
          ...
          jint di = mdo->bci_to_di(bci);                                       \
          SET_MDX(mdo->data_layout_at(di));                                    \
        }                                                                      \
      }                                                                        \
    } else {                                                                   \
      ...
      SET_MDX(mdo->data_layout_at(mdo->bci_to_di(BCI())));                     \
    }                                                                          \
  }
...
#define BI_PROFILE_CHECK_MDX()                                                 \
  {                                                                            \
    methodDataOop mdo = istate->method()->method_data();                       \
    address mdx  = (address) MDX();                                            \
    address mdx2 = (address) mdo->data_layout_at(mdo->bci_to_di(BCI()));       \
    guarantee(mdo  != NULL, "1");                                              \
...
/* FIXME: there is more to do here than increment and advance(mdx)! */         \
    ...
    SET_MDX(RetData::advance(mdo, bci));                                       \
  }
...
// Updates profiling data for a virtual call with given receiver klassOop.
#define BI_PROFILE_UPDATE_VIRTUALCALL(receiver)                                \
  if (ProfileInterpreter && MDX() != NULL) {                                   \
    BI_PROFILE_CHECK_MDX();                                                    \
