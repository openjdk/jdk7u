...
protected:
  ...
  static void set_oop_at(DataLayout* layout, int index, oop value) {
...
public:
  ...
  static klassOop receiver_unchecked(DataLayout* layout, uint row) {
    oop recv = oop_at(layout, receiver_cell_index(row));
    return (klassOop)recv;
  }

  static void increment_receiver_count_no_overflow(DataLayout* layout, klassOop rcvr) {
    ...
    for (int row = 0; row < num_rows; row++) {
      if (receiver_unchecked(layout, row) == NULL) {
        set_oop_at(layout, receiver_cell_index(row), rcvr);
        increment_uint_at_no_overflow(layout, receiver_count_cell_index(row));
        return;
      }
    }
...
#ifdef CC_INTERP
  static DataLayout* advance(methodDataOop mdo, int bci);
#endif // CC_INTERP
