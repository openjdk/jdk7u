...
address TemplateInterpreterGenerator::generate_return_entry_for(TosState state, int step) {
  ...
  Label Lgot_cache, Lgiant_index;

  if (EnableInvokeDynamic){
    __ lbz(R11_scratch1, 0, R14_bcp);
    __ cmpdi(CCR0, R11_scratch1, Bytecodes::_invokedynamic);
    __ beq(CCR0, Lgiant_index);
  }
  ...
  if (EnableInvokeDynamic){
    __ bind(Lgiant_index);
    __ get_cache_and_index_at_bcp(cache, 1, sizeof(u4));
    __ b(Lgot_cache);
  }
...
void TemplateInterpreterGenerator::generate_counter_incr(Label* overflow, Label* profile_method, Label* profile_method_continue) {
  ...
  if (TieredCompilation) {
    ...
    __ increment_invocation_counter(Rsum_ivc_bec, R12_scratch2);
    // Increment interpreter invocation counter.
    if (ProfileInterpreter) {  // %%% Merge this into methodDataOop.
      Register interpreter_invocation_counter_addr = R11_scratch1;
      __ lwz(R12_scratch2,in_bytes(methodOopDesc::interpreter_invocation_counter_offset()), R19_method);
      __ addi(R12_scratch2, R12_scratch2, 1);
      __ stw(R12_scratch2,in_bytes(methodOopDesc::interpreter_invocation_counter_offset()), R19_method);
    }
...
void TemplateInterpreterGenerator::lock_method(Register Rflags, Register Rscratch1, Register Rscratch2, bool flags_preloaded) {
  ...
  // Get synchronization object to Rscratch2.
  {
    ...
    __ ld(Robj_to_lock, in_bytes(methodOopDesc::const_offset()), R19_method);
    __ ld(Robj_to_lock, in_bytes(constMethodOopDesc::constants_offset()), Robj_to_lock);
    __ ld(Robj_to_lock, constantPoolOopDesc::pool_holder_offset_in_bytes(), Robj_to_lock);
...
//
// maxstack:   Max number of slots on the expression stack, loaded from the methodOop.
...
void TemplateInterpreterGenerator::generate_fixed_frame(bool native_call, Register Rsize_of_parameters, Register Rsize_of_locals) {
  ...
  __ lhz(Rsize_of_parameters /* number of params */, method_(size_of_parameters));
  if (native_call) {
    ...
    __ mr(Rsize_of_locals, Rsize_of_parameters);
  } else {
    __ lhz(Rsize_of_locals, method_(size_of_locals));
    __ sldi(Rsize_of_parameters, Rsize_of_parameters, Interpreter::logStackElementSize);
    __ sldi(Rsize_of_locals, Rsize_of_locals, Interpreter::logStackElementSize);
    __ lhz(top_frame_size, method_(max_stack));
  ...
  // set up R27_constPoolCache
  __ ld(R27_constPoolCache, in_bytes(methodOopDesc::const_offset()), R19_method);
  __ ld(R27_constPoolCache, in_bytes(constMethodOopDesc::constants_offset()), R27_constPoolCache);
  __ ld(R27_constPoolCache, constantPoolOopDesc::cache_offset_in_bytes(), R27_constPoolCache);
  ...
  if (ProfileInterpreter) {
    ...
    __ addi(R28_mdx, R28_mdx, in_bytes(methodDataOopDesc::data_offset()));
  ...
  if (native_call) {
    ...
    Register Rconst_method = R11_scratch1;
    __ ld(Rconst_method, method_(const)) ;
    __ add(R14_bcp,  in_bytes(constMethodOopDesc::codes_offset()), Rconst_method);
  }
...
address TemplateInterpreterGenerator::generate_empty_entry(void) {
  ...
  __ load_const_optimized(R11_scratch1, Interpreter::entry_for_kind(Interpreter::zerolocals), R12_scratch2);
  __ mtctr(R11_scratch1);
  __ bctr();
  __ flush();
...
inline bool math_entry_available(AbstractInterpreter::MethodKind kind) {
  if (!InlineIntrinsics) return false; // Generate a vanilla entry.
...
address TemplateInterpreterGenerator::generate_math_entry(AbstractInterpreter::MethodKind kind) {
  if (!math_entry_available(kind)) return NULL;
...
address TemplateInterpreterGenerator::generate_native_entry(bool synchronized) {
  ...
  generate_fixed_frame(true, size_of_parameters, R23_tmp3 /* out unused */);
  ...
  // If the method is static, get mirror into R4_ARG2.
  {
    ...
    __ ld(R11_scratch1, in_bytes(constMethodOopDesc::constants_offset()), R11_scratch1);
    // pool_holder = method->constants()->pool_holder();
    __ ld(R11_scratch1/*pool_holder*/, constantPoolOopDesc::pool_holder_offset_in_bytes(),
  ...
  // R19_method from state works; R19_method is invalid, because
...
address TemplateInterpreterGenerator::generate_normal_entry(bool synchronized) {
  ...
  if (synchronized) {
    ...
    Label Lok;
    __ lwz(R0, in_bytes(methodOopDesc::access_flags_offset()), R19_method);
...
void TemplateInterpreterGenerator::generate_throw_exception() {
  ...
  Interpreter::_remove_activation_preserving_args_entry = __ pc();
  {
    ...
    __ lhz(R4_ARG2, in_bytes(methodOopDesc::size_of_parameters_offset()), R19_method);
