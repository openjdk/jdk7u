...
void CppInterpreterGenerator::generate_compute_interpreter_state(Label& stack_overflow_return) {
  ...
  // parameter_count = method->size_of_parameters();
  // TODO: PPC port: assert(2 == methodOopDesc::sz_size_of_parameters(), "unexpected field size");
  __ lhz(parameter_count, method_(size_of_parameters));

  // local_count = method->max_locals();
  // TODO: PPC port: assert(2 == methodOopDesc::sz_max_locals(), "unexpected field size");
  __ lhz(local_count, method_(size_of_locals));

  // max_stack = method->max_stack();
  // TODO: PPC port: assert(2 == methodOopDesc::sz_max_stack(), "unexpected field size");
  __ lhz(max_stack, method_(max_stack));
  ...
  const Register local_addr    = R28_tmp8;
  {
    ...
    // if (!is_native) {
      ...
      __ addi(bytecode_addr, bytecode_addr, in_bytes(constMethodOopDesc::codes_offset()));
    // }
    __ bind(L);
  }

  __ ld(constants, in_bytes(methodOopDesc::const_offset()), R19_method);
  __ ld(constants, in_bytes(constMethodOopDesc::constants_offset()), constants);
  ...
  // cache = method->constants()->cache();
   __ ld(R0, constantPoolOopDesc::cache_offset_in_bytes(), constants);
  ...
  const int mirror_offset = in_bytes(Klass::java_mirror_offset());
  {
    ...
    __ ld(R11_scratch1/*pool_holder*/, constantPoolOopDesc::pool_holder_offset_in_bytes(), constants);
...
void CppInterpreterGenerator::generate_counter_incr(Label& overflow) {
  const Register invocation_counter    = R11_scratch1;
  const Register temp                  = R12_scratch2;
  const Register invocation_limit      = R12_scratch2;
  const Register invocation_limit_addr = invocation_limit;
  const Register access_flags          = R11_scratch1;

  // Update standard invocation counters
  __ increment_invocation_counter(invocation_counter, temp);
  ...
  __ cmpw(CCR0, invocation_counter, invocation_limit);
...
address CppInterpreterGenerator::generate_native_entry(void) {
...

  {
    ...
    __ ld(R11_scratch1/*constants*/, in_bytes(constMethodOopDesc::constants_offset()), R11_scratch1);
    // pool_holder = method->constants()->pool_holder();
...
address CppInterpreterGenerator::generate_normal_entry(void) {
  ...
  // Load callee methodOop, gc may have moved it.
  ...
  // Load BytecodeInterpreter._result._to_call._callee (callee's methodOop).
  __ ld(R19_method, state_(_result._to_call._callee));
...
void BytecodeInterpreter::layout_interpreterState(interpreterState to_fill,
  ...
  if (caller->is_interpreted_frame()) {
    ...
    //    care of popping arguments. Also, don't overwrite the MH.invoke methodOop in
    ...
    // The MethodHandleCompiler generates these synthetic methodOops,
