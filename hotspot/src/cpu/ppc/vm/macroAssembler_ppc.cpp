...
#ifdef _LP64
// Patch compressed oops constants.
int MacroAssembler::patch_set_narrow_oop(address a, address bound, long data) {
  ...
  narrowOop no = oopDesc::encode_heap_oop((oop)data);
...
// Get compressed constant.
int MacroAssembler::get_narrow_oop(address a, address bound) {
  assert(UseCompressedOops, "Should only patch compressed oops");
...
void MacroAssembler::check_klass_subtype_slow_path(Register sub_klass,
  ...
  bind(loop);
  if (UseCompressedOops) {
    // Oops in table are compressed.
    lwz(temp, base_offset, array_ptr);
    cmpw(CCR0, temp, super_klass);
  } else {
...
void MacroAssembler::biased_locking_enter(ConditionRegister cr_reg, Register obj_reg,
  ...
  load_heap_oop_with_trap_null_check(temp_reg, oopDesc::klass_offset_in_bytes(), obj_reg);
  ...
  orr(temp_reg, R16_thread, temp_reg);
  load_heap_oop_with_trap_null_check(temp2_reg, oopDesc::klass_offset_in_bytes(), obj_reg);
