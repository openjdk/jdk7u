...
static const bool support_IRIW_for_not_multiple_copy_atomic_cpu = true;
...
void TemplateTable::ldc(bool wide) {
  ...
  transition(vtos, vtos);
  Label notInt, notString, notClass, exit;
  ...
  __ cmpwi(CCR0, Rscratch2, JVM_CONSTANT_UnresolvedString);       // Unresolved string?
  ...
  __ align(32, 12);
  __ bind(notInt);
  __ cmpdi(CCR0, Rscratch2, JVM_CONSTANT_String);
  __ cmpdi(CCR1, Rscratch2, JVM_CONSTANT_Object);
  __ crnor(/*CR0 eq*/2, /*CR1 eq*/4+2, /*CR0 eq*/2);
  __ beq(CCR0, notString);

  __ isync(); // order load of constant wrt. tags
  __ ldx(R17_tos, Rcpool, Rscratch1);
...
void TemplateTable::fast_aldc(bool wide) {
  ...
  resolve_cache_and_index(f12_oop, R17_tos, Rcache, Rscratch, wide ? sizeof(u2) : sizeof(u1));

  __ verify_oop(R17_tos);

  const Register Rcon_klass = Rcache;
  const Register Rarray_klass = R4_ARG2;

  int ary_kl_offs = __ load_const_optimized(Rarray_klass, (address)Universe::systemObjArrayKlassObj_addr(), Rscratch, true);
  __ load_klass(Rcon_klass, R17_tos);
  __ ld(Rarray_klass, ary_kl_offs, Rarray_klass);

  __ cmpd(CCR0, Rarray_klass, Rcon_klass);
  __ bne(CCR0, Ldone);

  // Load array length.
  __ lwz(Rcon_klass, arrayOopDesc::length_offset_in_bytes(), R17_tos);
  __ cmpwi(CCR0, Rcon_klass, 0);
  __ beq(CCR0, LRet0);

  __ load_heap_oop(R17_tos, arrayOopDesc::base_offset_in_bytes(T_OBJECT), R17_tos);
...
void TemplateTable::branch(bool is_jsr, bool is_wide) {
  ...
  if (is_jsr) {
    ...
    __ ld(Rscratch1, in_bytes(methodOopDesc::const_offset()), R19_method);
    __ addi(Rscratch2, R14_bcp, -in_bytes(constMethodOopDesc::codes_offset()) + (is_wide ? 5 : 3));
  ...
  if (increment_invocation_counter_for_backward_branches) {
    ...
    if (TieredCompilation) {
      ...
      if (ProfileInterpreter) {
        ...
        __ ld(Rmdo, in_bytes(methodOopDesc::method_data_offset()), R19_method);
        __ cmpdi(CCR0, Rmdo, 0);
        __ beq(CCR0, Lno_mdo);

        // Increment backedge counter in the MDO.
        const int mdo_bc_offs = in_bytes(methodDataOopDesc::backedge_counter_offset()) + in_bytes(InvocationCounter::counter_offset());
      ...
      const int mo_bc_offs = in_bytes(methodOopDesc::backedge_counter_offset()) + in_bytes(InvocationCounter::counter_offset());
      __ bind(Lno_mdo);
      __ lwz(Rscratch2, mo_bc_offs, R19_method);
      __ load_const_optimized(Rscratch3, mask, R0);
      __ addi(Rscratch2, Rscratch2, increment);
      ...
      __ increment_backedge_counter(invoke_ctr, Rscratch2, Rscratch3);
...
void TemplateTable::ret() {
  ...
  __ ld(R11_scratch1, in_bytes(methodOopDesc::const_offset()), R19_method);
  __ add(R11_scratch1, R17_tos, R11_scratch1);
  __ addi(R14_bcp, R11_scratch1, in_bytes(constMethodOopDesc::codes_offset()));
  __ dispatch_next(vtos);
}
...
void TemplateTable::wide_ret() {
  ...
  __ ld(Rscratch1, in_bytes(methodOopDesc::const_offset()), R19_method);
  __ addi(Rscratch2, R17_tos, in_bytes(constMethodOopDesc::codes_offset()));
...
// Kills:
//   - Rscratch
void TemplateTable::resolve_cache_and_index(int byte_no, Register Rresult, Register Rcache, Register Rscratch, size_t index_size) {
  ...
  if (byte_no == f12_oop) {
    ...
    assert_different_registers(Rresult, Rcache);
    __ ld(Rresult, in_bytes(constantPoolCacheOopDesc::base_offset() + ConstantPoolCacheEntry::f1_offset()), Rcache);
    // Acquire by cmp-br-isync (see below).
    __ cmpdi(CCR0, Rresult, 0);
    __ bne(CCR0, Lresolved);
  } else {
    assert(byte_no == f1_byte || byte_no == f2_byte, "byte_no out of range");
  ...
  __ call_VM(noreg, entry, true);

  // Update registers with resolved info in case GC happened.
  __ get_cache_and_index_at_bcp(Rcache, 1, index_size);

  if (Rresult != noreg && byte_no == f12_oop) {
    // Need to reload resolved oop, GC could have happened during stepping back to java.
    __ ld(Rresult, in_bytes(constantPoolCacheOopDesc::base_offset() + ConstantPoolCacheEntry::f1_offset()), Rcache);
    __ twi_0(Rresult); // Acquire by twi-isync.
  } else {
    __ b(Ldone);
  }
...
void TemplateTable::load_invoke_cp_cache_entry(int byte_no,
  ...
  if (is_invokevfinal) {
    ...
    // Invokedynamic or plain methodhandle call.
    const size_t index_size = (is_invokedynamic ? sizeof(u4) : sizeof(u2));
    {
      Register Rscratch = Rmethod;
      resolve_cache_and_index(byte_no, Ritable_index, Rcache, Rscratch, index_size);
    }
    __ ld(Rmethod, index_offset, Rcache);
    __ ld(Rflags, flags_offset, Rcache);
  } else {
    {
      Register Rscratch = Rmethod;
...
void TemplateTable::getfield_or_static(int byte_no, bool is_static) {
  ...
  __ bne(CCR6, Lexit); // Non-volatile?
  __ fcmpu(CCR0, F15_ftos, F15_ftos); // Acquire by cmp-br-isync.
  __ beq_predict_taken(CCR0, Lacquire);
  __ b(Lacquire); // In case of NAN.
  ...
  __ bne(CCR6, Lexit); // Non-volatile?
  __ twi_0(R17_tos); // Acquire by cmp-twi-isync.
  __ b(Lacquire);
  ...
  __ bne(CCR6, Lexit); // Non-volatile?
  __ twi_0(R17_tos); // Acquire by cmp-twi-isync.
  // fallthru __ b(Lacquire);

  __ align(32, 12, 28); // Align Lexit.
  __ bind(Lacquire);
  __ isync(); // acquire
  __ bind(Lexit);
...
void TemplateTable::invokevirtual(int byte_no) {
  ...
  // Get return type. It's coded into the upper 4 bits of the lower half of the 64 bit value.
  __ rldicl(Rret_type, Rflags, 64-ConstantPoolCacheEntry::tos_state_shift, 64-ConstantPoolCacheEntry::tos_state_bits);
  __ load_dispatch_table(Rtable_addr, Interpreter::return_3_addrs_by_index_table());
  __ sldi(Rret_type, Rret_type, LogBytesPerWord);
  __ ldx(Rret_addr, Rret_type, Rtable_addr);
  __ null_check_throw(Rrecv, oopDesc::klass_offset_in_bytes(), R11_scratch1);
  __ load_heap_oop_not_null(Rrecv_klass, oopDesc::klass_offset_in_bytes(), Rrecv);
  __ verify_oop(Rrecv_klass);
...
void TemplateTable::invokevfinal_helper(Register RmethodOop, Register Rflags, Register Rscratch1, Register Rscratch2) {

  assert_different_registers(RmethodOop, Rflags, Rscratch1, Rscratch2);
  __ verify_oop(RmethodOop);

  // Load receiver from stack slot.
  Register Rrecv = Rscratch2;
  Register Rnum_params = Rrecv;

  __ lhz(Rnum_params, in_bytes(methodOopDesc::size_of_parameters_offset()), RmethodOop);
  ...
  // Do the call.
  __ call_from_interpreter(RmethodOop, Rret_addr, Rscratch1, Rscratch2);
}
...
void TemplateTable::invokespecial(int byte_no) {
  ...
  load_invoke_cp_cache_entry(byte_no, Rmethod, R11_scratch1, Rflags, /*virtual*/ false, false, false);

  __ verify_oop(Rmethod);
  __ lhz(R11_scratch1,  in_bytes(methodOopDesc::size_of_parameters_offset()), Rmethod);
  __ load_receiver(R11_scratch1, Rreceiver);
...
void TemplateTable::invokestatic(int byte_no) {
  ...
  load_invoke_cp_cache_entry(byte_no, R19_method, R11_scratch1, Rflags, /*virtual*/ false, false, false);
...
void TemplateTable::invokeinterface_object_method(Register Rrecv_klass,
  ...
  // Do the final call - the index (f2) contains the methodOop.
...
void TemplateTable::invokeinterface(int byte_no) {
  ...
  load_invoke_cp_cache_entry(byte_no, Rinterface_klass, Rindex, Rflags, /*virtual*/ false, false, false);
  // Get receiver from expression stack.
  Register Rparam_count = Rscratch1;
  __ andi(Rparam_count, Rflags, ConstantPoolCacheEntry::parameter_size_mask);
  __ load_receiver(Rparam_count, Rreceiver);
  __ verify_oop(Rreceiver);

  // Get return address.
  __ rldicl(Rret_type, Rflags, 64-ConstantPoolCacheEntry::tos_state_shift, 64-ConstantPoolCacheEntry::tos_state_bits);
  __ load_dispatch_table(Rtable_addr, Interpreter::return_5_addrs_by_index_table());
  __ sldi(Rret_type, Rret_type, LogBytesPerWord);
  __ ldx(Rret_addr, Rret_type, Rtable_addr);
...
// Common code for invokedynamic and invokehandle.
//
// Input:
//   - byte_no
//
// Output:
//   - Rmethod:        The method oop to invoke next.
//   - Rret_addr:      The return address to return to.
//   - Runextended_SP: The register the unextended SP will be saved in.
//   - Rappendix:      MethodType (invokehandle) or CallSite obj (invokedynamic).
//   - Rflags:         Method flags from const pool cache.
//   - Rrecv:          Cache for "this" pointer, might be noreg if static call.
//
//  Kills:
//   - Rscratch1
void TemplateTable::prepare_indy_invoke(int byte_no, Register Rmethod, Register Rret_addr, Register Runextended_SP,
                                        Register Rappendix, Register Rflags, Register Rrecv, Register Rscratch1) {
  // Determine flags.
  const Bytecodes::Code code = bytecode();
  const bool is_invokeinterface  = code == Bytecodes::_invokeinterface;
  const bool is_invokedynamic    = code == Bytecodes::_invokedynamic;
  ...
  load_invoke_cp_cache_entry(byte_no, Rmethod, Rappendix, Rflags, is_invokevirtual, false, is_invokedynamic);
  ...
  if (is_invokedynamic || is_invokehandle) {
    Label Ldone;
    __ verify_oop(Rappendix);
    __ rldicl_(R0, Rflags, 64-ConstantPoolCacheEntry::has_appendix_shift, 63);
    __ beq(CCR0, Ldone);
    // Push "appendix" (MethodType, CallSite, etc.).
    // This must be done before we get the receiver,
    // since the parameter_size includes it.
    __ push_ptr(Rappendix);
    __ bind(Ldone);
  }
  ...
  // Get return address.
  {
    Register Rtable_addr = Rscratch1;
    Register Rret_type = Rret_addr;
    address table_addr = (is_invokeinterface || is_invokedynamic) ?
      (address) Interpreter::return_5_addrs_by_index_table() : (address) Interpreter::return_3_addrs_by_index_table();
...
void TemplateTable::invokedynamic(int byte_no) {
  ...
  const Register Rcallsite      = R3_ARG1,
                 Rret_addr      = R4_ARG2,
                 Rflags         = R5_ARG3,
                 Rscratch1      = R11_scratch1,
                 Rscratch2      = R12_scratch2;
  ...
  prepare_indy_invoke(byte_no, R19_method, Rret_addr, R21_sender_SP, Rcallsite, Rflags, noreg, Rscratch1);
  ...
  __ call_from_interpreter(R19_method, Rret_addr, Rscratch1 /* scratch1 */, Rscratch2 /* scratch2 */);
}
...
void TemplateTable::invokehandle(int byte_no) {
  ...
  const Register Rmethod_type   = R3_ARG1,
                 Rret_addr      = R4_ARG2,
                 Rflags         = R5_ARG3,
                 Rrecv          = R6_ARG4,
                 Rscratch1      = R11_scratch1,
  ...
  prepare_indy_invoke(byte_no, Rmethod, Rret_addr, R21_sender_SP, Rmethod_type, Rflags, Rrecv, Rscratch1);
...
void TemplateTable::_new() {
  ...
  __ addi(Rtags, Rtags, typeArrayOopDesc::header_size(T_BYTE) * wordSize);
  ...
  __ addi(Roffset, Roffset, sizeof(constantPoolOopDesc));
  __ isync(); // Order load of instance Klass wrt. tags.
  __ ldx(RinstanceKlass, Roffset, Rcpool);
  ...
  __ cmpdi(CCR0, Rscratch, instanceKlass::fully_initialized);
  __ bne(CCR0, Lslow_case);

  // Make sure klass does not have has_finalizer, or is abstract, or interface or java/lang/Class.
  __ testbitdi(CCR0, R0, Rinstance_size, Klass::_lh_instance_slow_path_bit);
  __ bne(CCR0, Lslow_case);
  ...
  if (UseTLAB) {
    ...
    if (UseCompressedOops) {
      __ li(R0, 0);
      __ stw(R0, oopDesc::klass_gap_offset_in_bytes(), RallocatedObject); // klass gap if compressed
    }
    __ store_heap_oop_not_null(RinstanceKlass, oopDesc::klass_offset_in_bytes(), RallocatedObject); // klass (last for cms)
