...
void InterpreterMacroAssembler::get_const(Register Rdst) {
  ld(Rdst, in_bytes(methodOopDesc::const_offset()), R19_method);
}

void InterpreterMacroAssembler::get_constant_pool(Register Rdst) {
  get_const(Rdst);
  ld(Rdst, in_bytes(constMethodOopDesc::constants_offset()), Rdst);
}

void InterpreterMacroAssembler::get_constant_pool_cache(Register Rdst) {
  get_constant_pool(Rdst);
  ld(Rdst, constantPoolOopDesc::cache_offset_in_bytes(), Rdst);
}

void InterpreterMacroAssembler::get_cpool_and_tags(Register Rcpool, Register Rtags) {
  get_constant_pool(Rcpool);
  ld(Rtags, constantPoolOopDesc::tags_offset_in_bytes(), Rcpool);
}
...
void InterpreterMacroAssembler::unlock_if_synchronized_method(TosState state,
  ...
  Label Lunlocked, Lno_unlock;
  {
    ...
    lwz(Raccess_flags, in_bytes(methodOopDesc::access_flags_offset()), R19_method);
    li(R0, 0);
...
void InterpreterMacroAssembler::call_from_interpreter(Register Rtarget_method, Register Rret_addr, Register Rscratch1, Register Rscratch2) {
  ...
  ld(Rtarget_addr, in_bytes(methodOopDesc::from_interpreted_offset()), Rtarget_method);
  ...
  if (JvmtiExport::can_post_interpreter_events()) {
    ...
    beq(CCR0, done);
    ld(Rtarget_addr, in_bytes(methodOopDesc::interpreter_entry_offset()), Rtarget_method);
    align(32, 12);
    bind(done);
  }
...
void InterpreterMacroAssembler::set_method_data_pointer_for_bcp() {
  ...
  Label get_continue;
  ld(R28_mdx, in_bytes(methodOopDesc::method_data_offset()), R19_method);
  test_method_data_pointer(get_continue);
  call_VM_leaf(CAST_FROM_FN_PTR(address, InterpreterRuntime::bcp_to_di), R19_method, R14_bcp);

  addi(R28_mdx, R28_mdx, in_bytes(methodDataOopDesc::data_offset()));
...
void InterpreterMacroAssembler::verify_method_data_pointer() {
  ...
  ld(R12_scratch2, in_bytes(methodOopDesc::const_offset()), R19_method);
  addi(R11_scratch1, R11_scratch1, in_bytes(constMethodOopDesc::codes_offset()));
...
void InterpreterMacroAssembler::update_mdp_for_ret(TosState state,
   ...
   assert(ProfileInterpreter, "must be profiling interpreter");
   push(state);
   mr(R21_tmp1, return_bci); // Protect return_bci, in case it is volatile.
   call_VM(noreg, CAST_FROM_FN_PTR(address, InterpreterRuntime::update_mdp_for_ret), return_bci);
   mr(return_bci, R21_tmp1);
   pop(state);
}

// Increments the backedge counter.
// Returns backedge counter + invocation counter in Rdst.
void InterpreterMacroAssembler::increment_backedge_counter(const Register Rdst,
  ...
  lwz(counter, in_bytes(methodOopDesc::backedge_counter_offset()) + in_bytes(InvocationCounter::counter_offset()), R19_method);
  // Load invocation counter
  lwz(invocation_counter, in_bytes(methodOopDesc::invocation_counter_offset()) + in_bytes(InvocationCounter::counter_offset()), R19_method);
  ...
  stw(counter, in_bytes(methodOopDesc::backedge_counter_offset()) +
               in_bytes(InvocationCounter::counter_offset()), R19_method);
...
void InterpreterMacroAssembler::restore_interpreter_state(Register scratch, bool oops_only) {
  ld(scratch, 0, R1_SP);
  ld(R19_method, _ijava_state_neg(method), scratch);
  ld(R27_constPoolCache, _ijava_state_neg(cpoolCache), scratch);
  ld(R14_bcp, _ijava_state_neg(bcp), scratch); // related to method
  ld(R28_mdx, _ijava_state_neg(mdx), scratch); // related to method
  if (!oops_only) { // Following ones are stack addresses and don't require reload after GC.
...
#ifndef PRODUCT
  address pc = (address)(m->constMethod()) + in_bytes(constMethodOopDesc::codes_offset()) + bci;
